...
formBook.addEventListener("submit", function (event) {
  ...
  const id =
    JSON.parse(localStorage.setItem(storageKey)) === null
      ? 0 + Date.now()
      : JSON.parse(localStorage.setItem(storageKey)).length + Date.now();
  const newBook = {
    id: id,
    title: title,
    author: author,
...
function PutBookList(data) {
  if (CheckForStorage()) {
    ...
    if (localStorage.setItem(storageKey) !== null) {
      bookData = JSON.parse(localStorage.setItem(storageKey));
    }
